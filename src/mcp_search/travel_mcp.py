...
from mcp_search.travel_ferries import (
    ...
    build_search_workflows as ferry_build_workflows,
)
...
@mcp.tool()
async def travel_ferry_prices_via_safari(
    origin_port: str,
    dest_port: str,
    date: str,
    vehicle: str = "car",
    ...
    """Ferry prices via the user's desktop Safari (apple_browser_*).

    Ferry operators don't expose public pricing APIs and their booking
    sites are SPAs. Same workaround as Eurostar / Italo: build a
    pre-filled search URL, open it in the user's actual Safari (real
    fingerprint, no headless detection), wait for the page to render,
    then read prices off the page text.

    Returns one workflow entry per operator running the route — Channel
    routes typically come back as DFDS + P&O + Irish Ferries; some
    Brittany Ferries routes are single-operator.

    URL verification status is per-operator. The first time a tool call
    targets an operator whose URL pattern hasn't been confirmed
    (`url_verified: false`), the page may not pre-fill the form
    correctly — the calling LLM should fall back to "open the homepage,
    user fills the form, then read the rendered page". As URL shapes get
    confirmed, the verified flag flips to True (no code redeploy needed
    if the pattern matches; otherwise update _booking_url in
    travel_ferries.py).

    Args:
        origin_port: Substring of UK port name — 'Dover', 'Portsmouth',
                     'Holyhead', etc.
        dest_port: Substring of destination port — 'Calais', 'Caen',
                   'Dublin', etc.
        date: ISO date YYYY-MM-DD.
        vehicle: 'car' / 'high-vehicle' / 'caravan-trailer' /
                 'motorcycle' / 'foot-passenger'.
        passengers: Pax count (default 2).

    Workflow per operator entry the calling LLM should run:
      1. apple_browser_open_url(url=<search_url>)
      2. wait ~6 seconds for the SPA to render
      3. apple_browser_get_page_text()
      4. parse: each operator's results page lists sailings with
         times + prices (£NN or fromXNN) + cabin/class options
    """
    workflows = ferry_build_workflows(
    ...
    if not workflows:
        return json.dumps({
            "ok": False, "mode": "ferry",
            "error": f"no curated route between {origin_port!r} and {dest_port!r}",
            "hint": "Use travel_ferry_routes_to(<country>) to discover candidates.",
        }, indent=2)

    operators = []
    for w in workflows:
        verified = w.get("url_verified", False)
        if verified:
            ops_workflow = [
                {"step": 1, "tool": "mcp__claude_ai_Mees_Only__apple_browser_open_url",
                 "args": {"url": w["search_url"]},
                 "purpose": f"Pre-filled search at {w['operator']}"},
                {"step": 2, "tool": None, "wait_seconds": 6,
                 "purpose": "Let the SPA render results"},
                {"step": 3, "tool": "mcp__claude_ai_Mees_Only__apple_browser_get_page_text",
                 "args": {}, "purpose": "Read sailings + prices from the page"},
            ]
            workflow_note = None
        else:
            ops_workflow = [
                {"step": 1, "tool": "mcp__claude_ai_Mees_Only__apple_browser_open_url",
                 "args": {"url": w["search_url"]},
                 "purpose": f"Try the (unverified) pre-filled URL for {w['operator']}; "
                            "fall back to homepage if it 404s or shows error"},
                {"step": 2, "tool": None, "wait_seconds": 4,
                 "purpose": "Check if the page loaded results or an error"},
                {"step": 3, "tool": "mcp__claude_ai_Mees_Only__apple_browser_get_current_tab",
                 "args": {}, "purpose": "Confirm URL didn't redirect to error page"},
                {"step": 4, "tool": "mcp__claude_ai_Mees_Only__apple_browser_get_page_text",
                 "args": {}, "purpose": "Read prices if results rendered"},
                {"step": 5, "tool": None,
                 "purpose": (
                     "If page text doesn't contain prices, ask user to "
                     "complete the search by hand and rerun apple_browser_"
                     "get_page_text. Report back the working URL so it "
                     "can be added to _booking_url in travel_ferries.py."
                 )},
            ]
            workflow_note = (
                f"URL pattern for {w['operator']} is unverified — may not "
                "pre-fill the form. Fallback to manual form-fill is fine."
            )

        operators.append({
            "operator": w["operator"],
            "operator_key": w["operator_key"],
            "route": f"{w['origin_port']} → {w['dest_port']}",
            "country_to": w["country_to"],
            "crossing_minutes": w["crossing_minutes"],
    ...
    parsing_hints = {
        "price_pattern": r"(?:£|€|from\s+)\s?\d+(?:\.\d{2})?",
        "time_pattern": r"\d{2}:\d{2}",
        "duration_pattern": r"\d+\s*h(?:r|our)?s?\s*\d*\s*m(?:in)?",
        "common_classes": [
            "Standard", "Premium", "Premium Lounge", "Cabin",
            "Outside Cabin", "Inside Cabin", "Reclining Seat",
        ],
        "note": (
            "Channel ferries (90 min crossings) typically show flat per-"
            "vehicle prices. Long crossings (Brittany Ferries to Spain, "
            "DFDS Newcastle-Amsterdam, P&O Hull-Rotterdam) show separate "
            "vehicle + cabin pricing. Foot-passenger ferries (Steam Packet, "
            "some IE↔NI) are pax-only."
        ),
    }
    ...
    return json.dumps({
        "ok": True,
        "mode": "ferry",
        "service": "ferry-safari-pricecheck",
        "from": origin_port,
        "to": dest_port,
        "date": date,
        "vehicle": vehicle,
        ...
        "parsing_hints": parsing_hints,
        "data_sources": ["ferry-via-safari", "static-table"],
        "as_of": datetime.utcnow().isoformat() + "Z",
    }, indent=2)
