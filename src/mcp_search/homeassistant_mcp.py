"""MCP server for Home Assistant REST API access."""
...
_HA_URL = os.environ.get("HA_URL", "http://homeassistant:8123")
_HA_TOKEN = os.environ["HA_TOKEN"]
...
mcp = FastMCP("homeassistant", lifespan=ha_lifespan)
...
async def ha_entity_summary(
    ...
    """
    client = _client()
    resp = await client.get(f"{_HA_URL}/api/states")
    ...
    # Cap at 100 to avoid overwhelming output
...
async def ha_get_state(entity_id: str) -> str:
    ...
    """
    client = _client()
    resp = await client.get(f"{_HA_URL}/api/states/{entity_id}")
...
async def ha_history(
    ...
    states = data[0]
    rows = []
    for s in states[-100:]:  # last 100 changes
